...
int main(int argc, char **argv) {
	...
	//MPI_Comm_rank(workers, &sid);  /* My processor ID */

	MPI_Comm_rank(workers, &gid); //Global rank
	md = gid % 2; // = 1 (MD workers) or 0 (analysis workers)
	MPI_Comm_split(workers, md, 0, &workers);
	...
	for (stepCount = 1; stepCount <= StepLimit; stepCount++) {
		...
		if (stepCount%StepAvg == 0) {
			if (md) {
				//Send # of atoms, n, to rank gid - 1 in workers
				MPI_Send(&n, 1, MPI_INT, gid - 1, 1000, workers);
				//Send velocities of n atoms to rank gid - 1 in workers
				for (i = 0; i < n; i++)
					for (a = 0; a < 3; a++)
						dbuf[3 * i + a] = rv[i][a];
				MPI_Send(dbuf, 3*n, MPI_DOUBLE, gid - 1, 2000, workers);
				eval_props();
			}
			else {
				//Receive # of atoms, n, from rank gid + 1 in workers
				MPI_Recv(&n, 1, MPI_INT, gid + 1, 1000, workers,&status);
				//Receive velocities of n atoms from rank gid + 1 in workers
				MPI_Recv(dbufr, 3 * n, MPI_DOUBLE, gid + 1, 2000, workers,&status);
				for (i = 0; i < n; i++)
					for (a = 0; a < 3; a++)
						rv[i][a] = dbufr[3*i+a];
				calc_pv();
