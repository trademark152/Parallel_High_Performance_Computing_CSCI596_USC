...
Polynomial approximation of sqrt() function is used.

USAGE

%cc -o lmd_sqrt lmd_sqrt.c -lm
%lmd_sqrt < lmd.in (see lmd_sqrt.h for the input-file format)
*******************************************************************************/
#include <stdio.h>
#include <math.h>
#include "lmd_sqrt_flop.h"
#include <time.h>
...
int main(int argc, char **argv) {
	...
	printf("Number of FP operations = %le\n",flop_count);
	printf("MFlops rate = %le\n",mflops=flop_count/cpu/1.0e6);
	return 0;
}
			...
			while (i != EMPTY) {
				...
				while (j != EMPTY) {
					...
					if (i < j) {
						...
						if (rr < rrCut) {
							ri2 = 1.0/rr; ri6 = ri2*ri2*ri2;
							// r1 = sqrt(rr);
							// Polynomial-predictor + corrector for sqrt()
							rrn = rr*RRCUTI;
							fs = C0+rrn*(C1+rrn*(C2+rrn*C3)); // Polynomial with fused multiply-add (FMA)
							r1 = RCUT*(fs+0.5*(rrn/fs-fs));   // Newton-Raphson correction
