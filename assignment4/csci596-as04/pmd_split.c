...
/*--------------------------------------------------------------------*/
  ...
  MPI_Comm_split(MPI_COMM_WORLD,md,0,&workers);// dovode tp 2 subsets
  ...
  // NEW CODE: need modification by assigning to specfic ranl
  // page 3 splitMD note
  init_params();
  if (md){
    set_topology();
  ...
  for (stepCount=1; stepCount<=StepLimit; stepCount++) {
    ...
    if (stepCount%StepAvg == 0)
      if(md){
        ...
        MPI_Recv(&n,1,MPI_INT,gid+1,1000,MPI_COMM_WORLD,&status); //receive from next gid on the other side so gid+1
